...
def getData(indexOf, totalData, idCaleg, idPartai, idDapil, idPro):
	...
	try:
		...
		if (indexOf == '1'):
			...
			f = open('data_profil_caleg_prov.csv', 'a', encoding='utf-16')
			writer = csv.writer(f)
			writer.writerow(content)
...
if (TYPE_APPEND == '1') :
    ...
    message_string = "===== START :: APPENDING PROFIL CALEG DPRD PROV ––BY SINGLE ID idCaleg={}=====".format(ID_CALEG)
    print(message_string)
    pecker(LOG_IMAGE_CALEG, message_string)

    if (getData('1', '1', 'SINGLE', ''))
